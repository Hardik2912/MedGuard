...
import { safeMutation, safeStorageGet } from '../lib/safeAsync';
import { checkAllInteractions, getAdverseReactions, isAntibiotic } from '../lib/drugInteractions';
import { supabase } from '../lib/supabase';
import { checkMedicationRisk, extractPrescriptionWithAI } from '../lib/aiClient';
...
export default function ScanPrescription() {
    ...
    const compressImageDataUrl = (dataUrl, maxWidth = 1400, quality = 0.8) => {
    ...
    // --- REAL OCR via backend AI service ---
    const analyzeWithGemini = async () => {
        ...
        try {
            const ocrResult = await extractPrescriptionWithAI(base64Image);
            const medicines = ocrResult?.medicines || [];

            if (!medicines || medicines.length === 0) {
                setError(`Could not read this prescription clearly (source: ${ocrResult?.source || 'unknown'}). Try a brighter, straight photo.`);
                return;
            }
            setResults(medicines);
        } catch (err) {
            console.error('[OCR] Unexpected error:', err);
            setError('Could not process prescription right now. Please try again.');
        } finally {
            setAnalyzing(false);
        }
    };
    ...
    const runSafetyCheck = async () => {
        ...
        try {
            ...
            const backendRisk = await checkMedicationRisk({
                existingMedicines: (existingMeds || []).map(m => ({ name: m.name })),
                newMedicines: results,
                symptomIds: symptoms,
            });

            if (backendRisk) {
                setSafetyCheck({
                    level: backendRisk.level || 'green',
                    flags: backendRisk.flags || [],
                    redFlags: backendRisk.redFlags || 0,
                    yellowFlags: backendRisk.yellowFlags || 0,
                });
                setCheckingInteractions(false);
                return;
            }

            // Fallback to legacy local risk checks if backend is unavailable
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            const { success, error: dbError } = await safeMutation(
                'medicines',
                (sb) => sb.from('medicines').insert(medicinesToInsert)
            );

            if (success) {
                navigate('/dashboard');
            } else {
                setError('Failed to save medicines: ' + (dbError || 'Unknown error'));
            }
        } catch (err) {
            ...
            {!image ? (
                ...
                <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="relative rounded-2xl overflow-hidden shadow-lg">
                        ...
                        <button
                            onClick={() => { setImage(null); setBase64Image(null); setResults([]); setError(null); setSafetyCheck(null); }}
